...
Instance::Instance(const string& map_fname, const string& agent_fname,
                   ...
                   int num_of_rows, int num_of_cols, int num_of_obstacles,
                   int warehouse_width)
    : map_fname(map_fname),
...
void Instance::generateRandomAgents(int warehouse_width) {
  ...
  if (warehouse_width == 0)  // Generate agents randomly
  {
    ...
    while (k < num_of_agents) {
      ...
      while (goals[goal]) goal = randomWalk(goal, 1);
...
void Instance::printAgentTargets(pair<int, int> state) const {
  for (int i = 0; i < num_of_rows; i++) {
    for (int j = 0; j < num_of_cols; j++) {
      if (i == state.first && j == state.second)
        cout << 'X';
      else if (this->my_map[linearizeCoordinate(i, j)])
        cout << '@';
      else
        cout << '.';
    }
...
bool Instance::updateAgents(int agent_num,
  ...
  for (int i = 0; i < num_of_agents; i++) {
    ids[i] = i;
    start_locations[i] =
        linearizeCoordinate(agent_start[i].first, agent_start[i].second);
    goal_locations[i] =
        linearizeCoordinate(agent_goal[i].first, agent_goal[i].second);
  }
}
...
bool Instance::loadAgents() {
  ...
  if (line[0] == 'v')  // Nathan's benchmark
  {
    ...
    if (agent_indices != "") {
      ...
      for (int i = 0; i < num_of_agents; i++) ids[i] = i;
    }
    char_separator<char> sep("\t");
    int count = 0;
    int i = 0;
