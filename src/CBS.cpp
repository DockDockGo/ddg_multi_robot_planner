...
inline void CBS::updatePaths(CBSNode* curr) {
  for (int i = 0; i < num_of_agents; i++) paths[i] = &paths_found_initially[i];
...
void CBS::copyConflicts(const list<shared_ptr<Conflict>>& conflicts,
                        ...
                        const list<int>& excluded_agents) {
...
void CBS::findConflicts(CBSNode& curr, int a1, int a2) {
  ...
  if (paths[a1]->size() != paths[a2]->size()) {
    ...
    for (size_t timestep = min_path_length; timestep < paths[a2_]->size();
         timestep++) {
  ...
  if (curr.parent != nullptr) {
    ...
    for (auto it = new_agents.begin(); it != new_agents.end(); ++it) {
      ...
      for (int a2 = 0; a2 < num_of_agents; a2++) {
        ...
        if (!skip) findConflicts(curr, a1, a2);
      }
    }
  } else {
...
shared_ptr<Conflict> CBS::chooseConflict(const CBSNode& node) const {
  if (screen == 3) printConflicts(node);
  ...
  if (node.conflicts.empty() && node.unknownConf.empty())
    ...
    for (const auto& conflict : node.conflicts) {
      if (*choose < *conflict) choose = conflict;
    }
  } else {
...
void CBS::computePriorityForConflict(Conflict& conflict, const CBSNode& node) {
  conflict.secondary_priority = 0;  // random
                                    ...
                                    min(get<2>(conflict.constraint1.front()),
  ...
  while (!node.unknownConf.empty()) {
    ...
    if (type == constraint_type::EDGE)  // Edge conflict
    {
      cardinal1 = paths[a1]->at(timestep).is_single() &&
                  paths[a1]->at(timestep - 1).is_single();
      cardinal2 = paths[a2]->at(timestep).is_single() &&
                  paths[a2]->at(timestep - 1).is_single();
    } else  // vertex conflict or target conflict
    {
      if (!cardinal1) cardinal1 = paths[a1]->at(timestep).is_single();
      if (!cardinal2) cardinal2 = paths[a2]->at(timestep).is_single();
    }
    ...
    if (rectangle_helper.strategy != rectangle_strategy::NR &&
        ...
        (int)paths[con->a2]->size() >
            timestep &&  // conflict happens before both agents reach their goal
                         // locations
        type == constraint_type::VERTEX &&  // vertex conflict
        con->priority !=
            conflict_priority::CARDINAL)  // not caridnal vertex conflict
    {
    ...
    if (mutex_reasoning) {
      ...
      if (mutex_conflict != nullptr &&
          (*mutex_conflict != *con))  // ignore the cases when mutex finds the
...
void CBS::removeLowPriorityConflicts(
  ...
  for (const auto& conflict : conflicts) {
    int a1 = min(conflict->a1, conflict->a2),
        a2 = max(conflict->a1, conflict->a2);
...
bool CBS::findPathForSingleAgent(CBSNode* node, int ag, int lowerbound) {
  ...
  // find a path
  Path new_path = search_engines[ag]->findPath(*node, initial_constraints[ag],
                                               paths, ag, lowerbound);
...
bool CBS::generateChild(CBSNode* node, CBSNode* parent) {
  ...
  if (type == constraint_type::LEQLENGTH) {
    ...
    if ((int)node->constraints.size() ==
        2)  // generated by corridor-target conflict
    {
      int a =
          get<0>(node->constraints.back());  // it is a G-length constraint or a
                                             // range constraint on this agent
...
void CBS::saveResults(const string& fileName,
  ...
  if (!exist) {
    ...
    addHeads << "runtime,#high-level expanded,#high-level generated,#low-level "
...
void CBS::saveStats(const string& fileName, const string& instanceName) const {
  ...
  for (auto ins : heuristic_helper.sub_instances) {
    stats << get<0>(ins) << "," << get<1>(ins) << ","
          << get<2>(ins)->time_generated << "," << get<3>(ins) << ","
          << get<4>(ins) << endl;
  }
  stats.close();
  cout << "Write " << heuristic_helper.sub_instances.size()
       << " samples to files" << endl;
}
...
void CBS::saveCT(const string& fileName) const  // write the CT to a file
{
  ...
  for (auto node : allNodes_table) {
    ...
    output << node->parent->time_generated << " -> " << node->time_generated
           << " [label=\"";
    for (const auto& constraint : node->constraints) output << constraint;
    output << "\nAgents ";
    for (const auto& path : node->paths)
      output << path.first << "(+"
             << path.second.size() - paths_found_initially[path.first].size()
             << ") ";
    output << "\"]" << endl;
  }
  auto node = goal_node;
 ...
 * @return: retrive_path
 */
void CBS::getSolvedPaths(
    std::vector<std::vector<std::pair<int, int>>>& retrive_path) {
  retrive_path.clear();
  for (int i = 0; i < num_of_agents; i++) {
...
string CBS::getSolverName() const {
  string name;
  if (disjoint_splitting) name += "Disjoint ";
  ...
  if (target_reasoning) name += "+T";
  if (mutex_reasoning) name += "+M";
  if (bypass) name += "+BP";
  name += " with " + search_engines[0]->getName();
  return name;
}

bool CBS::solve(double _time_limit, int _cost_lowerbound,
                int _cost_upperbound) {
  ...
  try {
    ...
    while (!open_list.empty() && !solution_found) {
      ...
      if (runtime > time_limit || num_HL_expanded > node_limit ||
          heuristic_helper.sub_instances.size() >=
              MAX_NUM_STATS) {  // time/node out
        solution_cost = -1;
        solution_found = false;
        break;
      }
      ...
      if (curr->unknownConf.size() + curr->conflicts.size() ==
          0)  // no conflicts
      ...
      {
                      int a1 = 51, a2 = 54;
                      auto mdd1 = mdd_helper.getMDD(*curr, a1,
      paths[a1]->size()); cout << "The mdd of agent " << a1 << endl;
                      mdd1->printNodes();
                      auto mdd2 = mdd_helper.getMDD(*curr, a2,
      paths[a2]->size()); cout << "The mdd of agent " << a2 << endl;
                      mdd2->printNodes();
                      for (int t = 0; t < min(paths[a1]->size(),
      paths[a2]->size()); t++)
                      {
                                      if (paths[a1]->at(t).location ==
      paths[a2]->at(t).location) rectangle_helper.printOverlapArea(paths, t, a1,
      a2, mdd1, mdd2);
                      }
      ...
      while (foundBypass) {
        ...
        if (disjoint_splitting &&
            curr->conflict->type == conflict_type::STANDARD) {
          ...
          if (first)  // disjoint splitting on the first agent
          {
            ...
            if (type == constraint_type::VERTEX) {
              child[1]->constraints.emplace_back(
                  a, x, y, t, constraint_type::POSITIVE_VERTEX);
            } else {
              assert(type == constraint_type::EDGE);
              child[1]->constraints.emplace_back(
                  a, x, y, t, constraint_type::POSITIVE_EDGE);
            }
          ...
          if (curr->conflict->type == conflict_type::RECTANGLE &&
              rectangle_helper.strategy == rectangle_strategy::DR) {
            int i = (bool)(rand() % 2);
            for (const auto constraint : child[1 - i]->constraints) {
              child[i]->constraints.emplace_back(
                  get<0>(constraint), get<1>(constraint), get<2>(constraint),
                  get<3>(constraint), constraint_type::POSITIVE_BARRIER);
            }
          } else if (curr->conflict->type == conflict_type::CORRIDOR &&
                     corridor_helper.getStrategy() == corridor_strategy::DC) {
            int i = (bool)(rand() % 2);
            assert(child[1 - i]->constraints.size() == 1);
            auto constraint = child[1 - i]->constraints.front();
            child[i]->constraints.emplace_back(
                get<0>(constraint), get<1>(constraint), get<2>(constraint),
                get<3>(constraint), constraint_type::POSITIVE_RANGE);
          }
        }
        ...
        for (int i = 0; i < 2; i++) {
          ...
          if (child[i]->g_val + child[i]->h_val == min_f_val &&
              curr->unknownConf.size() + curr->conflicts.size() ==
                  0)  // no conflicts
          {           // found a solution (and finish the while look)
            break;
...
CBS::CBS(const Instance& instance, bool sipp, int screen)
    : screen(screen),
      ...
      heuristic_helper(instance.getDefaultNumberOfAgents(), paths,
                       search_engines, initial_constraints, mdd_helper) {
  clock_t t = clock();
  initial_constraints.resize(
      num_of_agents, ConstraintTable(instance.num_of_cols, instance.map_size));
...
bool CBS::generateRoot() {
  ...
  if (paths_found_initially.empty()) {
    ...
    for (auto i : agents) {
      ...
      paths_found_initially[i] = search_engines[i]->findPath(
          *dummy_start, initial_constraints[i], paths, i, 0);
  ...
  for (int a1 = 0; a1 < num_of_agents; a1++) {
    for (int a2 = a1 + 1; a2 < num_of_agents; a2++) {
      ...
      for (size_t timestep = 0; timestep < min_path_length; timestep++) {
        ...
        int loc2 = paths[a2]->at(timestep).location;
        if (loc1 == loc2) {
          cout << "Agents " << a1 << " and " << a2 << " collides at " << loc1
          ...
          return false;
                   ...
                   loc2 == paths[a1]->at(timestep + 1).location) {
          cout << "Agents " << a1 << " and " << a2 << " collides at (" << loc1
               << "-->" << loc2 << ") at timestep " << timestep << endl;
          return false;
        }
      }
      if (paths[a1]->size() != paths[a2]->size()) {
